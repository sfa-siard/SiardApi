...
public class MimeTypesGenerator {
    private static final File fileJava = new File("src/ch/enterag/utils/mime/MimeTypes.java");
    ...
    private MimeTypesGenerator() {
        ...
        try {
            ...
            for (String sLine = readLine(rdrMimeTypes); sLine != null; sLine = readLine(rdrMimeTypes)) {
                ...
                if (iComment >= 0)
                    sLine = sLine.substring(0, iComment)
                                 .trim();
                if (sLine.length() > 0) {
                    int iTab = sLine.indexOf('\t');
                    if (iTab >= 0) {
                        String sMimeType = sLine.substring(0, iTab)
                                                .trim();
                        String sExtensions = sLine.substring(iTab + 1)
                                                  .trim();
            ...
            wrJava.close();
            System.out.println();
            System.out.println(iMimeTypes + " MIME type extensions generated.");
